...
M.availability_siakadpaid.form.initInner = function(config) {
    this.config = config || {prodis: [], defaults: {}};
};
...
M.availability_siakadpaid.form.getNode = function(json) {
    ...
    var html = '<div class="availability-group d-flex flex-wrap gap-2 align-items-center">' +
        '<label>' + M.util.get_string('label_prodi', 'availability_siakadpaid') + ' ' +
        '<select name="prodi" class="form-select d-inline-block w-auto">' +
    ...
    for (var i = 0; i < (config.prodis || []).length; i++) {
        html += '<option value="' + Y.Escape.html(config.prodis[i].code) + '">' +
            Y.Escape.html(config.prodis[i].name) + '</option>';
    }
    html += '</select></label>' +
        '<label>' + M.util.get_string('label_tahunajaran', 'availability_siakadpaid') + ' ' +
        '<input class="form-control d-inline-block w-auto" type="text" name="tahunajaran"></label>' +
        '<label>' + M.util.get_string('label_semester', 'availability_siakadpaid') + ' ' +
        '<select class="form-select d-inline-block w-auto" name="semester">' +
        '<option value="">-</option><option value="ganjil">' + M.util.get_string('semester_ganjil', 'availability_siakadpaid') + '</option>' +
        '<option value="genap">' + M.util.get_string('semester_genap', 'availability_siakadpaid') + '</option>' +
        '</select></label>' +
        '<label>' + M.util.get_string('label_jenis', 'availability_siakadpaid') + ' ' +
        '<input class="form-control d-inline-block w-auto" type="text" name="jenis" placeholder="' +
    ...
    node.all('select,input').on('change', function() {
